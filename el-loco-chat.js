...
// Version 5.3 - Fixed inventory search and parts pricing
...
(function() {
    ...
    function searchVehicles(searchQuery) {
        ...
        const cleanedQuery = searchQuery.toLowerCase()
            ...
            .replace(/chevy/gi, 'chevrolet')
            .replace(/transmission for a|transmission for|transmission|engine for a|engine for|engine|parts for a|parts for|parts|for a|for/g, '')
            .trim();
        ...
        const results = inventoryData.filter(vehicle => {
            ...
            // Check if ALL words in the query are in the vehicle string
            const searchWords = cleanedQuery.split(/\s+/).filter(word => word.length > 0);
            
            // If searching for just a year, return all vehicles from that year
            if (searchWords.length === 1 && /^\d{4}$/.test(searchWords[0])) {
                return vehicle.year === searchWords[0];
            }
            
            // For other searches, all words must match
    ...
    // ========================
    // FIXED PARTS PRICE SEARCH
    // ========================
    function searchPartPrice(query) {
        const cleanedQuery = query.toLowerCase()
            .replace(/how much for a|how much for an|how much for|how much is a|how much is an|how much is|price of a|price of an|price of|cost of a|cost of an|cost of|price for a|price for an|price for|what's the price of a|what's the price of an|what's the price of|what's the price for a|what's the price for an|what's the price for/g, '')
            .replace(/\?/g, '')
            .trim();
        ...
        // Try to find matches that contain the search term
        const matches = [];
        for (let partName in PARTS_PRICES) {
            if (partName.includes(cleanedQuery) || cleanedQuery.includes(partName)) {
                matches.push({ name: partName, ...PARTS_PRICES[partName] });
            }
        }

        // If we have exact one match, return it
        if (matches.length === 1) {
            return matches[0];
        }
        ...
        if (matches.length > 1) {
            ...
            const exactWordMatch = matches.find(match => {
                const words = match.name.split(/\s+/);
                return words.some(word => word === cleanedQuery);
            });
    ...
    async function processMessage(msg) {
        ...
        // Check for price queries with better regex
        const pricePattern = /(?:how much|price|cost|what's the price)/i;
        const partPattern = /(?:for a |for an |for |is a |is an |is )(.+?)(?:\?|$)/i;
        
        if (pricePattern.test(lower)) {
            // Check if asking for general parts list
            if (lower.includes('parts') && !partPattern.test(lower.replace('parts', ''))) {
                return `
            ...
            // Try to extract the specific part
            const partMatch = lower.match(partPattern);
            if (partMatch && partMatch[1]) {
                const partQuery = partMatch[1].trim();
                const part = searchPartPrice(partQuery);
                if (part) {
        ...
        // Handle vehicle searches - Fixed to work with "honda civic" etc
        const vehicleKeywords = /honda|civic|accord|toyota|camry|corolla|ford|f150|f-150|mustang|chevrolet|chevy|silverado|nissan|altima|dodge|ram|jeep|mazda|hyundai|kia|volkswagen|audi|bmw|mercedes|lexus|infiniti|acura|subaru|mitsubishi|gmc|buick|cadillac|chrysler|lincoln|pontiac|saturn|scion|suzuki|volvo|mini|fiat|alfa|tesla|rivian|lucid|\d{4}/i;
        
        if (vehicleKeywords.test(lower)) {
            // Check if it's asking about parts for a specific vehicle
            const partKeywords = /transmission|engine|alternator|starter|door|fender|hood|bumper|radiator|battery|tire|wheel|brake|window|mirror|seat|axle|differential/i;
            
            if (partKeywords.test(lower)) {
                const partMatch = lower.match(partKeywords);
                const partName = partMatch ? partMatch[0] : 'part';
                
                // Extract vehicle info
                const vehicleQuery = lower.replace(partKeywords, '').replace(/for a |for an |for |do you have a |do you have an |do you have |any /g, '').trim();
                
                const results = searchVehicles(vehicleQuery);
                
                if (results.length > 0) {
                    let response = `🔧 <strong>Great news! We have ${results.length} matching vehicle(s), so we should have that ${partName} for you!</strong><br><br>`;
                    response += `<em>Note: We inventory whole vehicles, not individual parts. You'll need to pull the ${partName} yourself.</em><br><br>`;
                    ...
                    return `😕 <strong>No ${vehicleQuery} in the yard right now for that ${partName}.</strong><br><br>${getRandomResponse('noInventory')}<br><br>Or try searching for a different vehicle!`;
                }
            } else {
                // Just searching for vehicles
        ...
        if (lower.includes('inventory')) {
            ...
            return `We have ${inventoryData.length || 'tons of'} vehicles in our yard! Tell me what you're looking for! Try: "Honda Civic" or "2015 Ford F150"`;
        }
        ...
        return `🤔 I'm not quite sure about that, amigo. Try asking me something like:<br><br>
        • "Do you have a Honda Civic?" (vehicle search)<br>
        • "How much for an alternator?" (part price)<br>
        • "Transmission for 2015 Ford F150" (part + vehicle)<br>
        • "Store hours" or "Directions"<br><br>
